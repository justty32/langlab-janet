...
> 完全沒碰過 Janet？先走 [course/](../course/README.md)，這裡是查資料用的密集版。

每篇程式碼都在 Janet 1.41.2 實測過。00 → 06 依序讀完就能上手；07 之後每篇獨立，需要時再翻。

> **從 C／C++／Lua／Go／Python 過來的？** 每種都有逐條對照的一組篇章
> （[01d](01d-提早離開-return-break-continue.md)、[43](43-從-C-C++-過來.md)、
> [44](44-從-Lua-過來.md)、[45](45-從-Go-過來.md)、[46](46-從-Python-過來.md)）
> → **[從別的語言過來 · 索引](從別的語言過來-索引.md)**；順序見 [路線圖](路線圖.md)。
> **想的是「我現在要做這件事」而不是「屬於哪一章」？** → **[我要做 X，去哪](怎麼做-X.md)**
> ——按任務排的索引，每列給「教學／可跑／可抄」三欄。
...
| 01d | [提早離開：return、break、continue](01d-提早離開-return-break-continue.md) | ★ 沒有 `return`／`continue`：最後運算式就是回傳值、函式裡 `break` 即 return、⚠ **`(break 值)` 在迴圈裡值被丟掉**、`label` 跳巢狀 |
...
從零寫一個會用工具的 agent，七篇依序讀。**全部離線可跑**（範例自帶假伺服器，
不用金鑰）。封裝好的成品在 [`modules/agent/`](../modules/agent/README.md)。

| # | 篇 | 重點 |
|---|----|------|
| 47 | [LLM API 到底是什麼](47-llm-api-是什麼.md) | ★ 它就是一個 HTTP POST：送一份 JSON、回一份 JSON，沒有連線也沒有狀態 |
| 47b | [system prompt 與多輪對話](47b-多輪對話.md) | 模型不記得你——「記憶」是你自己維護的陣列，每輪整份重送 |
| 47c | [tool calling 白話](47c-tool-calling.md) | ★ 模型不會執行任何東西，它只回「請幫我叫這個函式」的 JSON，權限全在你手上 |
| 47d | [自己寫一個 agent loop](47d-自己寫-agent-loop.md) | agent 就是一個 while：問 → 執行工具 → 餵回去 → 再問，十七行 |
| 47e | [圖像輸入](47e-圖像輸入.md) | 圖不是另開欄位，是把 `content` 從字串換成 parts 陣列，圖走 data URI |
| 47f | [錯誤與成本](47f-錯誤與成本.md) | 六種狀況一張表，⚠ **其中三種不會丟例外** |
| 47g | [接真後端](47g-接真後端.md) | 換成真的只要改 `:url` 與金鑰；三條路的取捨、金鑰放哪、上線前檢查表 |

## 需要時再翻的三區

**語言細節（32–40）**——條件與 `match`／`loop` 全表／函式參數與閉包／讀錯誤訊息／
拷貝與凍結／走訪巢狀資料／排序／效能實測／型別全表／作業系統／內建動態變數。
每篇的 ⚠ 都是實測出來、猜不到的行為 → [語言細節索引](語言細節索引.md)。

**從別的語言過來（01b、01d、43–46）**——C／C++、Lua、Go、Python 各一組逐條對照，
每篇的 ⚠ 都是「照原本那樣寫會錯」的地方 → [從別的語言過來 · 索引](從別的語言過來-索引.md)。
...
> 想快速查：**[`cheatsheets/`](../cheatsheets/README.md)** 是分頁速查表（核心／資料IO／PEG／並行／C互通／env），`html/index.html` 可全文搜尋。
> 撞到怪行為想確認「是不是已知的坑」→ **[`cheatsheets/地雷.md`](../cheatsheets/地雷.md)**：全部實測過的地雷集中一頁。
> 可跑範例在 [`examples/`](../examples/README.md)（教學附件）與 [`snippets/`](../snippets/README.md)（做事的起點）。

## 怎麼讀這份教學

**閱讀順序**（最快上手路線、從 C++ 過來的路線、⚠／★ 標記的意思）見 [路線圖](路線圖.md)。
