...
（小提醒：`(/ 1 0)` 在 Janet 是 `inf` 不是錯誤，別拿它試 protect，見 [21](21-數字與位元.md)。）

錯誤處理的完整用法——結構化錯誤、`assert`、`defer`／`with` 資源管理、何時該回 `nil` 而不是拋錯
——在 [20-錯誤處理與資源管理.md](20-錯誤處理與資源管理.md)。這篇只講「它底層是 fiber」這件事。
