...
讀 code map 找到相關領域（只讀清單裡的檔）
  → 增量修改（守 conventions）
  → jpm test 綠燈；改到 .md 就再跑 wf-lint
  → 宣稱「Windows 上會怎樣」的改動 → 交使用者實機驗 → 回報問題 → 修 → 重複
  → 全數通過後：更新 code map → 補文檔 → commit
```

- 自動驗證是 agent 自己跑的把關；跑不了的一律交使用者（見 [testing](../testing.md) 的「誰跑」欄）。這台是 Manjaro，**agent 驗得了的只有 Linux 行為**。
